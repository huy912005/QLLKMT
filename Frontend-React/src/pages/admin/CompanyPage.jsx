...
const CompanyFormModal = ({ isOpen, onClose, onSubmit, initialData }) => {
    const isEdit = !!initialData?.idCongTy;
    const [form, setForm] = useState({ tenCongTy: "" });

    useEffect(() => {
        if (initialData) {
            setForm({ tenCongTy: initialData.tenCongTy || "" });
        } else {
            setForm({ tenCongTy: "" });
        }
    }, [initialData, isOpen]);
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000}}>
            <div style={{ background: "#fff", borderRadius: "12px", width: "400px", overflow: "hidden" }}>
                <div style={{ background: "#1e293b", padding: "16px 20px", color: "#fff", display: "flex", justifyContent: "space-between" }}>
                    <h3 style={{ margin: 0 }}>{isEdit ? "Sửa Công Ty" : "Thêm Công Ty"}</h3>
                    <button onClick={onClose} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer" }}>✕</button>
                </div>
                <div style={{ padding: "20px" }}>
                    <label style={{ fontSize: "14px", fontWeight: "bold", display: "block", marginBottom: "8px" }}>Tên công ty / Hãng <span style={{color:"red"}}>*</span></label>
                    <input 
                        type="text" 
                        value={form.tenCongTy} 
                        onChange={(e) => setForm({ tenCongTy: e.target.value })} 
                        placeholder="Ví dụ: Intel, AMD, ASUS, MSI..."
...
const CompanyPage = () => {
    ...
    const handleRemove = async (id) => {
        const result = await Swal.fire({ title: "Xác nhận xóa?", text: "Xóa công ty có thể ảnh hưởng đến sản phẩm liên kết!", icon: "warning", showCancelButton: true });
        if (result.isConfirmed) {
            try {
                await companyService.remove(id);
                Swal.fire("Thành công", "Đã xóa hãng sản xuất", "success");
                fetchCompanies();
    ...
    const handleModalSubmit = async (formData) => {
        if (!formData.tenCongTy) return Swal.fire("Lỗi", "Vui lòng nhập tên công ty", "error");
        try {
            if (editingCompany) {
                await companyService.update(editingCompany.idCongTy, formData);
            } else {
                await companyService.create(formData);
            }
            Swal.fire("Thành công", editingCompany ? "Đã cập nhật" : "Đã thêm mới", "success");
            setShowModal(false);
            fetchCompanies();
        } catch (err) { Swal.fire("Lỗi", err.message, "error"); }
    };

    const filtered = companies.filter(c => c.tenCongTy.toLowerCase().includes(filterText.toLowerCase()));
    ...
    return (
        <div style={{ padding: "24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                <h2 style={{ margin: 0, fontSize: "22px" }}>Quản lý hãng (Công ty)</h2>
                <button onClick={() => { setEditingCompany(null); setShowModal(true); }} style={{ background: "#4bc26d", color: "#fff", border: "none", padding: "8px 16px", borderRadius: "8px", display: "flex", gap: "6px", cursor: "pointer", fontWeight: "bold" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead style={{ background: "#1e293b", color: "#fff", textAlign: "left" }}>
                        <tr>
                            ...
                            <th style={{ padding: "12px 16px" }}>Tên hãng</th>
                            <th style={{ padding: "12px 16px" }}>Hành động</th>
                        </tr>
                    </thead>
                    <tbody>
                        {paginated.map(c => (
                            <tr key={c.idCongTy} style={{ borderBottom: "1px solid #f1f5f9", transition: "0.2s" }}>
                                <td style={{ padding: "12px 16px", fontWeight: "bold", color: "#64748b" }}>{c.idCongTy}</td>
                                <td style={{ padding: "12px 16px", fontWeight: "500" }}>{c.tenCongTy}</td>
                                <td style={{ padding: "12px 16px", display: "flex", gap: "8px" }}>
                                    <button onClick={() => { setEditingCompany(c); setShowModal(true); }} style={{ background: "#3b82f6", color: "#fff", border: "none", padding: "6px", borderRadius: "6px", cursor: "pointer" }}><Pencil size={16} /></button>
                                    <button onClick={() => handleRemove(c.idCongTy)} style={{ background: "#ef4444", color: "#fff", border: "none", padding: "6px", borderRadius: "6px", cursor: "pointer" }}><Trash2 size={16} /></button>
                                </td>
                            </tr>
                        ))}
                        {paginated.length === 0 && <tr><td colSpan="3" style={{ padding: "20px", textAlign: "center", color: "#94a3b8" }}>Không tìm thấy dữ liệu nào</td></tr>}
                    </tbody>
                </table>
            </div>
            ...
            <CompanyFormModal isOpen={showModal} onClose={() => setShowModal(false)} onSubmit={handleModalSubmit} initialData={editingCompany} />
        </div>
    );
};
